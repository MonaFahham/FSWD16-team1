...
if (isset($_POST["submit"])) {
    ...
    if ($pictureArray->error === 0) {
        ...
        $sql = "UPDATE user SET first_name = '$first_name', last_name = '$last_name', email = '$email', address = '$address', phone_number = '$phone_number',  date_of_birth = '$date_of_birth', picture = '$pictureArray->fileName',status='$status' WHERE id = {$id}";
    } else {
        $sql = "UPDATE user SET first_name = '$first_name', last_name = '$last_name', email = '$email', address = '$address', phone_number = '$phone_number',  date_of_birth = '$date_of_birth',status='$status' WHERE id = {$id}";
    }
...
<?php require_once './components/navbar.php' ?>
    ...
    <p class="title mt-3 text-center display-5 " style="color:#A58F6F; margin-bottom:10%">Edit your Profile: </p>
        <legend class='h2'>Update request <img class='img-thumbnail rounded-circle' src='picture/<?php echo $picture ?>' alt="<?php echo $first_name ?> <?php echo $last_name ?>"></legend>
        <form method="post" enctype="multipart/form-data">
            <table class="table" style="margin-bottom:10%">
                ...
                <td> <select name="status">
                        ...
                        <option <?php if($status == "adm") { echo "selected";}  ?> value="adm">Admin</option>
                        <option <?php if($status == "ban") { echo "selected";}  ?> value="ban">banned</option>
                    </select>
                </td>
                </tr>
